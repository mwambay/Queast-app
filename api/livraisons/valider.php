...
try {
    ...
    // 2. Historique du statut
    $stmt = $pdo->prepare("
        INSERT INTO order_status_history
        (order_id, status, changed_by, notes)
        VALUES (?, 'delivered', ?, ?)
    ");
    $stmt->execute([$orderId, $currentUser['id'], 'Livraison validée par le livreur']);

    $pdo->commit();

    // Journalisation
    error_log("Livraison validée: commande $orderId par livreur {$currentUser['id']}");
